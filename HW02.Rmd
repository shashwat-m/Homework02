...
ggplot(bikes_9am, aes(x = factor(weathersit), y = avg_total, fill = factor(weathersit))) +
  ...
  labs(title = "Average Ridership at 9 AM by Weather Conditions", x = "Weather Condition", y = "Average Ridership", fill = "Weather Conditions") + scale_fill_manual(values = c("1" = "navyblue", "2" = "lightblue", "3" = "skyblue"),labels = c("1" = "Clear Conditions", "2" = "Misty Conditions/Partly Cloudy","3" = "Precipitation")) + facet_wrap(~ workingday, scales = "free_y", labeller = labeller(workingday = c("0" = "Not Working Day", "1" = "Working Day"))) + theme_minimal() + theme(legend.position = "bottom")
```
...
ggplot(artists_with_ten_week_hits, aes(x = reorder(performer, ten_week_hits), y = ten_week_hits)) +
  ...
  labs(
    title = "Top 19 Artists with at Least 30 Ten-Week Hits (1958-2021)",
    x = "Artist",
